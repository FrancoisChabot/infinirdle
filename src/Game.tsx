...
const pickWord = (prev:string[]):string => {
    while(true) {
...
type LetterState = "basic" | "good" | "bad" | "misplaced";
...
const LetterDisplay = (props: {small?:boolean, letter: string, state: LetterState})=>{
    let classToUse = styles.letter;
    if(props.small) {
        classToUse = styles.smallLetter;
    }
    return <div className={`${classToUse} ${styles[props.state]}`}>
        {props.letter}
    </div>;
}

const GuessDisplay = (props:{ small?:boolean, entries:{l:string, s:LetterState}[]})=>{
   
    
    return <div className={styles.letterRow}>
        {props.entries.map((e,i)=>
            <LetterDisplay small={props.small} key={i} letter={e.l} state={e.s}/>
        )}
    </div>;
};

const HistoricGuessDisplay = (props:{ guess:string; answer:string; small?:boolean})=>{
    const entries: {l:string, s:LetterState}[] = [{
        l: "",
        s: "basic"
    },{
        l: "",
    ...
    const l_count : Record<string, number> = {}
    ...
    for(let i=0;i< props.guess.length; ++i) {
     ...
     for(let i=0;i< props.guess.length; ++i) {
        ...
        entries[i].l = c;
        if(c!== props.answer[i]) {
            if(!(c in l_count) || l_count[c] === 0) {
                entries[i].s = "bad";
            }
            else {
                l_count[c] -= 1;
                entries[i].s = "misplaced";
            }
        }
     }

     return <GuessDisplay entries={entries} small={props.small}/>;
}

const ActiveDisplay = (props:{ guess:string})=>{
    const entries: {l:string, s:LetterState}[] = [{
        l: "",
        s: "basic"
    },{
    ...
    for(let i=0;i< props.guess.length; ++i) {
        entries[i].l = props.guess[i];
    }
    return <GuessDisplay entries={entries}/>;
}
const ActivePuzzle = (props: {puzzle: Puzzle; onGuess:(guess:string)=>void})=>{
    ...
    const advance = (key:string) => {
        if(key === "ENTER") {
            ...
            if( key.length === 1 && key.toUpperCase() !== key.toLowerCase() &&current.length < 5) {
                setCurrent(c=>c + key);
            }
        }
    };
    ...
    return <div>
        {props.puzzle.guesses.map((h,i)=><HistoricGuessDisplay key={i} guess={h} answer={props.puzzle.answer}/>)}
        <ActiveDisplay guess={current}/>
        <Keyboard onKey={advance}/>
    </div>;
};
const HistoryDisplay = (props:{history: Puzzle[]})=>{
    return <div className={styles.historyContainer}>
        {props.history.map((h,i)=><><div key={i}>
            {h.guesses.map((g,j)=><HistoricGuessDisplay small key={j} guess={g} answer={h.answer}/>)}
        
        </div>
        <div className={styles.vsep}></div></>)}
    </div>
}
...
const Game = ()=>{
    ...
    const [rem, setRem] = useState(g);

    const advance = (guess:string)=>{
        const new_current: Puzzle = {
            answer: current.answer,
            guesses: [...current.guesses, guess]
        };

        if(guess === current.answer || rem === 1) {
            setHistory(h=>[...h, new_current]);
            const prevGuesses = [...current.guesses, guess];
            setCurrent({answer:pickWord(prevGuesses), guesses:prevGuesses});
        }
        else {
    ...
    return <div>
        <h1>Welcome to Infinirdle!</h1>
        You have a total of {g} guesses. Try to solve as many puzzles as possible!
        <hr/>
        You have solved {history.length} puzzles so far!
        <HistoryDisplay history={history}/>
        <hr/>
        You have {rem} guesses remaining!
        {rem > 0 ? <ActivePuzzle puzzle={current} onGuess={advance}/> : <h2>it's over!</h2>}
